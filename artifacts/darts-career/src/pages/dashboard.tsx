...
import {
  ...
  ArrowUp, ArrowDown, Minus, Flame, Star,
} from "lucide-react";
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center border-b border-border pb-3">
                    ...
                    <div className="flex items-center gap-2">
                      {(career as any).rang_veraenderung != null && (career as any).rang_veraenderung !== 0 && (
                        <motion.div
                          initial={{ opacity: 0, scale: 0.7 }}
                          animate={{ opacity: 1, scale: 1 }}
                          className={`flex items-center gap-0.5 text-sm font-bold px-1.5 py-0.5 rounded ${
                            (career as any).rang_veraenderung > 0
                        ...
                        >
                          ...
                          {Math.abs((career as any).rang_veraenderung)}
                        </motion.div>
                      )}
                      {(career as any).rang_veraenderung === 0 && (career as any).letzter_rang != null && (
            ...
            {/* Rivalitätssystem */}
            {(() => {
              const rivalen = (career as any).rivalitaeten;
              const angst = rivalen?.angstgegner ?? [];
              const liebling = rivalen?.lieblingsgegner ?? [];
              if (angst.length === 0 && liebling.length === 0) return null;
              return (
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h3 className="text-xl font-display font-bold">Rivalitätssystem</h3>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {angst.length > 0 && (
                      <div>
                        <p className="text-xs text-muted-foreground font-semibold uppercase tracking-widest mb-2 flex items-center gap-1.5">
                          <AlertCircle className="w-3 h-3 text-red-400" /> Angstgegner
                        </p>
                        <div className="space-y-2">
                          {angst.map((r: any) => (
                            <div key={r.name} className="flex items-center justify-between rounded-xl border border-red-500/20 bg-red-500/5 px-3 py-2">
                              <span className="font-medium text-sm text-white truncate">{r.name}</span>
                              <span className="text-xs font-mono text-red-300 shrink-0 ml-2">{r.siege}S / {r.niederlagen}N</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {liebling.length > 0 && (
                      <div>
                        <p className="text-xs text-muted-foreground font-semibold uppercase tracking-widest mb-2 flex items-center gap-1.5">
                          <Star className="w-3 h-3 text-yellow-400" /> Lieblingsgegner
                        </p>
                        <div className="space-y-2">
                          {liebling.map((r: any) => (
                            <div key={r.name} className="flex items-center justify-between rounded-xl border border-yellow-400/20 bg-yellow-400/5 px-3 py-2">
                              <span className="font-medium text-sm text-white truncate">{r.name}</span>
                              <span className="text-xs font-mono text-yellow-300 shrink-0 ml-2">{r.siege}S / {r.niederlagen}N</span>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            {/* Gegner-Reaktion nach letztem Match */}
            {(career as any).letzte_gegner_reaktion && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Reaktion nach dem Match</span>
                </div>
                <div className="flex gap-3 items-start">
                  <div className="shrink-0 w-9 h-9 rounded-full bg-secondary/80 border border-border flex items-center justify-center text-base">
                    💬
                  </div>
                  <div>
                    <p className="text-sm font-bold text-white mb-1">{(career as any).letzte_gegner_reaktion.name}</p>
